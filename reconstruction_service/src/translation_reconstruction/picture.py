...
def apply_pixel_rotation(img_obj, entry: Dict):
    ...
    `_pixel_rotation_deg` is `page_rot` from `_derotate_rotated_pages`'s
    rotation-snap convention (COUNTER-CLOCKWISE-positive degrees the CONTENT
    is turned from upright — see geometry.py's `normalize_rotation`), i.e.
    "90.0" means the content reads bottom-to-top and needs a CLOCKWISE turn
    to straighten. `PIL.Image.rotate(angle)` is CCW-positive (matplotlib/math
    convention), so straightening takes the NEGATED angle: `rotate(-90)` is
    what turns "reads bottom-to-top" content CW into upright. Verified against
    a real document: `rotate(90)` (no sign flip) produced an upside-down
    mirror image; `rotate(-90)` matched the source drawing exactly, and
    matches the CW bbox remap `_map()` already applies for the same case.
    """
