...
# e.g. "Noto Sans Symbols2", "Segoe UI Symbol" on Windows.
...
# (\u2611/\u2610/\u2612 checkboxes, \u25cb/\u25cf/\u25ef radios, \u2713/\u2714/\u2717/\u2718 ticks). These live in the Unicode
# "Miscellaneous Symbols"/"Dingbats" blocks \u2014 NOT in Latin text faces like
# Calibri/Arial \u2014 so a run like "\u2611 Normal \u2610 Machine changeover" carries no CJK
# yet still has no glyph in its Latin font and the box silently drops in Word /
# strict renderers. We route such runs' eastAsia slot to a symbol-bearing CJK
# face (SimSun et al. all include these), the same escape valve used for CJK.
_RE_FORM_SYMBOL = re.compile(r"[\u2610-\u2612\u25cb\u25cf\u25ef\u2713\u2714\u2717\u2718]")
...
def resolve_east_asia_font(font: str, text: str, explicit: Optional[str]) -> str:
    ...
    - When the text has a ballot/form symbol (☑/☐/○/✓…) that a Latin-only
      ...
      this, translated (Latin-script) forms silently lose their checkboxes.
    """
    if explicit:
        return explicit
...
def build_run_xml(text: str, style: Optional[Dict]) -> str:
    ...
    # too so the glyph is guaranteed to draw. CJK runs are unaffected (they keep
    # their Latin face and rely on eastAsia as before).
    ascii_font = font
    if (
