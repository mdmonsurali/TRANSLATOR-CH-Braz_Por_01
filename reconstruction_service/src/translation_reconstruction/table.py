...
# ocr_reconstruction so this rule cannot drift between the two copies.
...
def parse_table_grid(
    ...
    elif max_cols > (Counter(widths).most_common(1)[0][0] if widths else 0):
        log.info(
            "[table] grid widened to %d cols: rowspan coverage from earlier "
            "rows means most rows span more columns than their own cell count "
            "(modal cell count was %d)",
            max_cols, Counter(widths).most_common(1)[0][0] if widths else 0,
        )
...
# one when the translated text genuinely can't fit the original table size.
#
...
# silently land on the floor. The table is allowed to overflow its source bbox
# slightly instead (see `_TABLE_FONT_OVERFLOW_TOLERANCE`) — a table one row
# taller than its OCR rectangle reads far better than one nobody can read.
# Override per deployment with DOCX_TABLE_MIN_FONT_PT.
#
...
def render_table(
    ...
    if pictures_for_table and img_by_anchor:
        _ins = _dropped_column_from_pictures(
            pictures_for_table, assign_bbox, cell_anchors, img_by_anchor,
    ...
    # Floored at `_TABLE_TARGET_FONT_PT`: on a many-column table the derived cap
    # (`narrowest / 5`) collapses to a few points before a single cell has been
    # measured, which pinned dense tables at 7pt no matter how much room they
    # had. The shrink loop below is the real no-clip guarantee, so starting at
    # the target size costs nothing — it just stops the heuristic from
    # pre-emptively shrinking text that would have fitted.
    ...
    if True:
        ...
        # A residual overflow here no longer escalates to growth: the table is
        # fixed, so the 3.5pt-floor render stands.
        _any_cell_overflows(
            cell_anchors, col_w_emus, row_h_pts, max_cols, n_rows,
